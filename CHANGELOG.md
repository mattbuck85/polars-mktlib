...
- **Cross-call consistency via shared seeds.** Identical *seed* values across :func:`var` and :func:`cvar` calls produce identical sample paths under the perf path (``independent_streams=False``). Reporting code achieves "chart and tail-risk numbers from the same simulation" by threading one fixed seed through every MC call — no module-level cache, no fingerprint hashing.
...
- **No MC cache.** Earlier drafts of v0.11.0 introduced an opt-in module-level cache keyed on a content fingerprint of the input series.  That layer was removed before release: the perf-path single-batch sampler (10k×22 ≈ 10–15 ms) is fast enough that re-running MC on every call is cheaper than cache bookkeeping plus the collision-risk surface that any fingerprint scheme inevitably carries.  Callers who want consistent paths across multiple metrics share a *seed* instead — the deterministic RNG does the work, no cache state needed.
