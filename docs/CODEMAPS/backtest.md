...
| `PriceExpr` | `_conditions.py:22` | Arithmetic building block: `Col`, `Lit`, `Pct`, `_BinOp` |
...
2. Resolve entry/exit conditions to boolean columns (auto-wrap bare `pl.Expr` → `Custom`)
3. Build `_position` (1=in, 0=out) with forward-fill
