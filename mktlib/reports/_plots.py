...
def cumulative_returns_chart(
    ...
    fig = fig.update_layout(_base_layout("Cumulative Returns"), yaxis_tickformat=".1%")
    return _to_div(fig)
...
def yearly_returns_chart(years: list[int], returns: list[float]) -> str:
    ...
    fig = fig.update_layout(_base_layout("Yearly Returns", height=350), yaxis_tickformat=".1%")
    return _to_div(fig)
...
def monthly_heatmap_chart(years: list[int], months: list[int], returns: list[float]) -> str:
    ...
    fig = go.Figure(data=go.Heatmap(
        z=z, x=month_labels, y=[str(y) for y in unique_years],  # type: ignore[arg-type]  # stubs too narrow for Heatmap
        text=text, texttemplate="%{text}",  # type: ignore[arg-type]
        colorscale="RdYlGn", zmid=0,
        hovertemplate="Year: %{y}<br>Month: %{x}<br>Return: %{text}<extra></extra>",
    ))
    fig = fig.update_layout(_base_layout("Monthly Returns", height=max(200, len(unique_years) * 35 + 100)))
    return _to_div(fig)
...
def daily_returns_scatter(dates: list[dt.date], returns: list[float]) -> str:
    ...
    fig = fig.add_hline(y=0, line_dash="dash", line_color="gray", opacity=0.5)
    fig = fig.update_layout(_base_layout("Daily Returns"), yaxis_tickformat=".2%")
    return _to_div(fig)


def returns_distribution_chart(returns: list[float]) -> str:
    fig = go.Figure()
    fig = fig.add_trace(go.Histogram(x=returns, nbinsx=50, marker_color=STRATEGY_COLOR, opacity=0.7, name="Returns"))
    fig = fig.update_layout(_base_layout("Returns Distribution", height=350), xaxis_tickformat=".1%", bargap=0.05)
    return _to_div(fig)
